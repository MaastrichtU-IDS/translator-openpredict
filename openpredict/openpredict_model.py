...
def predictDrugRepositioning(diseaseCURIElist,noofResults):
  ...
  try:
    ...
    with open(cwd+FILEPATH+"kgpredict_drug_diseasemappings.tsv", newline='', encoding='utf-8') as csvfile:
    ...
    entity_idmap_file = cwd+EMBPATH+'entities.tsv'
    relation_idmap_file = cwd+EMBPATH+'relations.tsv'
...
def getSHAPModel(datasetFile="/openpredict/data/deepdrug_repurposingpredictiondataset.csv",mlmodel="LR"):
    cwd = os.getcwd()   
...
def getXPREDICTExplanation(shap_values,drugId="000"):
    ...
    cwd = os.getcwd()   
    #plt.show()
    datasetFile="/openpredict/data/deepdrug_repurposingpredictiondataset.csv"
    deepdrug_dataset_df=pd.read_csv(cwd+datasetFile)
...
def get_explanations(
    ...
    # TODO: improve when we will have more classifier
    predictions_array = query_omim_drugbank_classifier(id_to_predict, model_id)
    shap_valueslr= getSHAPModel(datasetFile="/openpredict/data/deepdrug_repurposingpredictiondataset.csv")
