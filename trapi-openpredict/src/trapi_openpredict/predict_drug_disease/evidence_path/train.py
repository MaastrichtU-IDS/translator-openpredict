...
from trapi_openpredict.predict_drug_disease.utils import load_features_embeddings, get_openpredict_dir
...
def calculateEntitySimilarities(tokenized_vector, topn = 100) :
    ...
    entities = list(tokenized_vector.get_normed_vectors())
    similarity_scores = []
...
def percentiles_of_different_features():
    ...
    for feature in features_drug :
        ...
        f'{get_openpredict_dir()}embedding/feature_specific_embeddings_KG/feature_FeatureTypesDrugs.{feature!s}.txt', binary=False)
    ...
    for feature in features_diseases :
        ...
        f'{get_openpredict_dir()}embedding/feature_specific_embeddings_KG/feature_FeatureTypesDiseases.{feature!s}.txt', binary=False)
...
def save_embedding_as_txt(embedding_df, fileName) :
    ...
    np.savetxt(f'{get_openpredict_dir()}/evidence-path-model/feature_{fileName}', embedding_df_np, fmt = '%f' )
