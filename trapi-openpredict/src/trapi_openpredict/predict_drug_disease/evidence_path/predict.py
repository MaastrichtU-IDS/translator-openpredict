...
from trapi_openpredict.predict_drug_disease.evidence_path.train import getQuantiles, path_weight_product
from trapi_openpredict.predict_drug_disease.utils import get_openpredict_dir
...
def generate_paths_for_apair(drug, disease, drug_emb_vectors, disease_emb_vectors,features_drug = None, features_disease = None,threshold_drugs = 1,threshold_diseases = 1):
    ...
    # similarDrugs = drug_emb_vectors.similar_by_key(drug, topn=100) if drug in drug_emb_vectors else []
    # similarDiseases = disease_emb_vectors.similar_by_key(disease, topn=100) if disease in disease_emb_vectors else []
