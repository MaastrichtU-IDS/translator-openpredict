...
from trapi_openpredict.predict_drug_disease.utils import createFeaturesSparkOrDF, load_features_embeddings, load_similarity_embeddings, get_openpredict_dir, resolve_ids_with_nodenormalization_api, resolve_id
...
@trapi_predict(
    path='/predict-drug-disease',
    name="Get drug-treats-disease predictions",
...
def query_omim_drugbank_classifier(input_curie, model_id):
    ...
    loaded_model = load(f"{get_openpredict_dir()}../models/{model_id}")
...
def get_similar_for_entity(input_curie: str, emb_vectors, n_results: int = None):
    ...
    #g= Graph()
    if not n_results:
        n_results = len(emb_vectors.get_normed_vectors())
...
@trapi_predict(
    path='/similarity-drug-disease',
    name="Get similar entities for a given drug or disease",
