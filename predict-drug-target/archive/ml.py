...
def cv_run(run_index, pairs, classes, embedding_df, train, test, fold_index, clfs):
    ...
    train_df = pd.DataFrame(
        list(zip(pairs[train, 0], pairs[train, 1], classes[train])), columns=["Drug1", "Drug2", "Class"]
    )
    test_df = pd.DataFrame(
        list(zip(pairs[test, 0], pairs[test, 1], classes[test])), columns=["Drug1", "Drug2", "Class"]
    )

    train_df = train_df.merge(embedding_df, left_on="Drug1", right_on="Drug").merge(
        embedding_df, left_on="Drug2", right_on="Drug"
    )
    test_df = test_df.merge(embedding_df, left_on="Drug1", right_on="Drug").merge(
        embedding_df, left_on="Drug2", right_on="Drug"
    )
