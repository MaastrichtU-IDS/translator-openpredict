...
def adjcencydict2matrix(df, name1, name2):
    ...
    print (len(df))
    a = df.pivot_table(index=name1, columns=name2, aggfunc = 'sum')
    print("successful")
    return a
...
def mergeFeatureMatrix(drugfeatfiles, diseasefeatfiles):
    for i,featureFilename in enumerate(drugfeatfiles):
        ...
        df.set_index(['Drug1', 'Drug2'], append=True)
        df = df.drop_duplicates()
    ...
    for i,featureFilename in enumerate(diseasefeatfiles):
        ...
        df=pd.read_csv(featureFilename, delimiter=',')
        df = df.drop_duplicates()
        df.set_index(['Disease1', 'Disease2'], append=True)
...
def evaluate(test_df, clf):
    ...
    scores = sklearn.model_selection.cross_validate(clf, X_test,y_test,scoring = scoring)
    return scores
