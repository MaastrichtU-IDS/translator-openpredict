...
# oc exec $pod_id wget -N http://download3.openlinksw.com/uda/vad-vos-packages/7.2/conductor_dav.vad
...
# oc exec $pod_id isql -U dba -P $DBA_PASSWORD exec="vad_install ('conductor_dav.vad', 0);"
oc exec $pod_id -- isql-v -U dba -P $DBA_PASSWORD exec="vad_install ('ods_framework_dav.vad', 0);"
oc exec $pod_id -- isql-v -U dba -P $DBA_PASSWORD exec="vad_install ('ods_briefcase_dav.vad', 0);"

# Create /DAV/ldp folder: http://docs.openlinksw.com/virtuoso/fn_dav_api_add/
oc exec $pod_id -- isql-v -U dba -P $DBA_PASSWORD exec="select DB.DBA.DAV_COL_CREATE ('/DAV/ldp/','110100100R', 'dav','dav','dav', '${DBA_PASSWORD}');"
